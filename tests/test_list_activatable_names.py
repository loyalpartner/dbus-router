...
def test_list_activatable_names_basic(test_log_dir: Path, router_env):
    """ListActivatableNames should return a list of activatable service names."""
    with router_env(EMPTY_CONFIG, socket_prefix="lan_") as env:
        _, _, router_addr = env
        result = asyncio.run(
...
def test_list_activatable_names_merged(test_log_dir: Path, router_env):
    """ListActivatableNames should merge results from both buses."""
    with router_env(HOST_ROUTE_CONFIG, socket_prefix="lan_") as env:
...
def test_list_activatable_names_no_duplicates(test_log_dir: Path, router_env):
